...
modificado = (
    leido.split()
)  # como divide con espacios no puede detectar texto con espacios
# --------------------------------------------------------------------------------------------------
...
class Tabladesimbolos:
    def __init__(
        self, pos, tipo, lexema, expresiones, arbol, expsemantica, arbseman, tipodato
    ):
        self.pos = pos
        self.tipo = tipo
        self.lexema = lexema
            ...
            case 2:  # int
                ...
                for i in range(len(valoresNumericos)):  # Es valor numerico valido?
                    ...
                    if tablita[pasos].tipo == valoresNumericos[i]:
                        pasos += 1
                        break
                    if i < len(valoresNumericos):
                        continue

                    ErrorSintactico(
                        "Valor numerico (Numero o ID)", tablita[pasos].lexema
                    )
                ...
                if tablita[pasos].tipo == OpM:
                    ...
                    if tablita[pasos].tipo == OpM:
                        pasos += 1
                        for i in range(
                            len(valoresNumericos)
                        ):  # Es valor numerico valido?
...
def infijo_a_postfijo(expresion):
    ...
    for token in expresion:
        if token.isdigit() or token.isalpha() or token.isidentifier():
            ...
            while (
                pila and pila[-1] != "(" and precedencia(pila[-1]) >= precedencia(token)
            ):
                salida.append(pila.pop())
            pila.append(token)
    ...
    for i in range(len(tablita)):
        ...
        if tablita[i].tipo == "ID" and tablita[i].expresiones != None:
            ...
            for i2 in range(posicionID + 1, len(tablita)):
                ...
                if (
                    tablita[i2].tipo == "ID"
                    and tablita[i2].expresiones == None
                    and tablita[posicionID].lexema == tablita[i2].lexema
                ):
...
def AgregarTipodec():
    ...
    for i in range(len(tablita)):
        ...
        if (
            tablita[i].tipo == "decimal"
            and tablita[i - 2].tipo == "ID"
            and tablita[i + 1] == ";"
        ):
        ...
        if (
            tablita[i].tipo == "NUM"
            and tablita[i - 2].tipo == "ID"
            and tablita[i - 3].tipo == "float"
            and tablita[i + 1].tipo == ";"
        ):
            tablita[i].tipodato = "float"
            tablita[i - 2].tipodato = "float"
            continue
    ...
    for i in range(len(tablita)):
        ...
        if tablita[i].tipo == "ID" and tablita[i].expresiones != None:
            ...
            if longitud > 1:
                ...
                tablita[i].arbol = construir_arbol_desde_prefijoN(
                    tablita[i].expresiones
                )
...
def Expresionsemantica():
    ...
    for i in range(len(tablita)):
        if tablita[i].expresiones != None:
            ...
            if (
                len(exp) < 2
                and tablita[i].tipodato == "int"
                or tablita[i].tipodato == "float"
            ):
...
def comprobar_tipos(raiz):
    ...
    if raiz.valor in "+-*/^":
        if tipo_izq not in ["int", "float"] or tipo_der not in ["int", "float"]:
            raise TypeError(
                f"Operación {raiz.valor} no soportada para tipos {tipo_izq} y {tipo_der}"
            )
    ...
    # Operadores de comparación (ejemplo adicional)
        ...
        if tipo_izq != tipo_der:
            raise TypeError(
                f"Comparación {raiz.valor} requiere tipos iguales, no {tipo_izq} y {tipo_der}"
            )
        raiz.tipo = "bool"
...
def AddArbolsem():
    for i in range(len(tablita)):
        if tablita[i].expsemantica != None:
            ...
            if lon > 1:
                tablita[i].arbseman = construir_arbol_desde_prefijo(
                    tablita[i].expsemantica
                )
                tablita[i].tipodato = comprobar_tipos(tablita[i].arbseman)
    ...
    for i in range(len(tablita)):
        ...
        if tablita[i].lexema == "while" or tablita[i].lexema == "if":
            ...
            if var1.tipodato != var2.tipodato:
                print(
                    "Las variables son de tipos diferentes no se puede realizar la comparacion:  -",
                    var1.tipodato,
                    "-   -",
                    var2.tipodato,
                    "-",
